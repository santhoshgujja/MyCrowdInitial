        ...
        private void LoadData(string elementPath,string filetype)
        {
            ...
            foreach (string file in files)
            {
                ...
                else if (filetype.ToLower().Contains("document"))
                {
                    ...
                    {
                        ListviewData.Items.Add(fileName);
                        ListviewData.Items[index].ImageIndex = 0;
                        index++;
                    }
                    else if (fileName.ToLower().Contains(".xls") || fileName.ToLower().Contains(".xlsx"))
                    {
                        ListviewData.Items.Add(fileName);
                        ListviewData.Items[index].ImageIndex = 1;
                        index++;
                    }
                    else if (fileName.ToLower().Contains(".ppt") || fileName.ToLower().Contains(".pptx"))
                    {
                        ListviewData.Items.Add(fileName);
                        ListviewData.Items[index].ImageIndex = 2;
                        index++;
                    }
                    else if (fileName.ToLower().Contains(".pdf") || fileName.ToLower().Contains(".pdf"))
                    {
                        ListviewData.Items.Add(fileName);
                        ListviewData.Items[index].ImageIndex = 6;
                        index++;
                    }
                }
